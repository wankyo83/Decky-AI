...
Steam Deck과 SteamOS 기반 UMPC의 Decky Loader에서 실행되는 게임 도우미 플러그인입니다. Gemini 대화·화면 분석, 로컬 Whisper 음성 타이핑, YouTube 공략 검색을 게임을 종료하거나 데스크톱으로 전환하지 않고 빠른 메뉴에서 사용할 수 있습니다.
...
마이크 버튼은 질문을 자동으로 전송하는 기능이 아니라 가상 키보드 대신 문장을 입력하는 기능입니다.

1. `AI 대화하기`에서 메시지 입력란 오른쪽의 마이크 버튼을 누릅니다.
2. 입력할 내용을 말합니다.
3. 마이크 버튼을 다시 누르면 녹음이 끝나고 로컬 음성 인식이 시작됩니다.
4. 인식된 문장이 메시지 입력란에 추가됩니다.
5. 내용을 확인하거나 수정한 뒤 사용자가 직접 `질문 보내기`를 누릅니다.

음성 인식에는 Gemini API를 사용하지 않습니다. [`whisper.cpp`](https://github.com/ggml-org/whisper.cpp)의 CPU 실행 파일과 한국어를 지원하는 다국어 `small-q5_1` 모델을 기기에서 직접 실행합니다. 따라서 Gemini 무료 사용량을 소진했거나 인터넷 연결이 끊어진 상태에서도, 최초 준비가 끝난 뒤에는 음성 타이핑을 사용할 수 있습니다.

### 최초 다운로드

- 최초 녹음을 시작할 때 `다운로드 약 200MB` 안내를 표시합니다.
- 최초 녹음을 끝내면 whisper.cpp 실행 파일 약 9MB와 음성 모델 약 190MB를 한 번만 다운로드합니다.
- 다운로드 중에는 `받은 용량 / 전체 용량`과 전체 예상 용량을 메시지 입력창 위에 표시합니다.
- 다운로드가 중단되면 불완전한 파일을 삭제하고 다음 시도 때 다시 받습니다.
- 이후 음성 입력에서는 저장된 실행 파일과 모델을 재사용하므로 다시 다운로드하지 않습니다.

### 저장되는 파일과 삭제되는 파일

| 파일 | 처리 방식 |
|---|---|
| 마이크 녹음 WAV | 인식 성공·실패·취소 후 자동 삭제 |
| 임시 변환 TXT | 결과를 입력란에 반영한 뒤 자동 삭제 |
| 중단된 다운로드 임시 파일 | 오류 또는 취소 시 자동 삭제 |
| whisper.cpp 실행 파일 약 9MB | 다음 음성 입력을 위해 설정 폴더에 유지 |
| `small-q5_1` 모델 약 190MB | 다음 음성 입력을 위해 설정 폴더에 유지 |

녹음된 음성은 Gemini, Google 또는 다른 외부 서버로 전송되지 않습니다. 변환할 때만 최대 CPU 4스레드로 Whisper 프로세스를 실행하며, 변환이 끝나거나 Decky AI 창을 닫으면 프로세스를 종료합니다.
...
음성 인식 구성과 파일 정리 방식은 [로컬 Whisper 음성 타이핑](#로컬-whisper-음성-타이핑)을 참고하세요.
...
Decky AI 코드는 BSD-3-Clause 라이선스로 배포됩니다. 자세한 내용은 [LICENSE](LICENSE)를 확인하세요.

음성 인식에 사용하는 [whisper.cpp](https://github.com/ggml-org/whisper.cpp)는 MIT 라이선스이며, 최초 준비 시 공식 GitHub 릴리스에서 실행 파일을 다운로드합니다. Whisper 모델은 [ggerganov/whisper.cpp 모델 저장소](https://huggingface.co/ggerganov/whisper.cpp)에서 다운로드합니다.
