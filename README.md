...
- 직접 선택할 수 있는 `일반 대화(자동)` / `항상 웹 검색` 모드
- 화면 키보드와 마이크 버튼을 함께 제공하는 질문 입력창
- 현재 게임 용어를 힌트로 사용하는 한국어 음성 인식
...
평소에는 3.5 Flash를 사용합니다. 질문에 날씨, 기온, 미세먼지, 뉴스, 환율, 주가, 실시간 가격 같은 표현이 포함되면 자동으로 2.5 Flash Google 검색으로 전환합니다.
...
| `파주시 문산읍 오늘 날씨 알려줘` | 2.5 Flash + Google Search |
| `최신 패치 기준 추천 장비 알려줘` | 2.5 Flash + Google Search |

### 항상 웹 검색 모드

모든 질문을 2.5 Flash + Google Search로 보냅니다. 최신 정보 확인에는 유용하지만 검색 한도를 더 빨리 사용할 수 있고 일반 질문보다 응답 시간이 길어질 수 있습니다.
...
- 음성 입력: SteamOS PipeWire의 `pw-record`
...
`429`는 주로 분당 요청 수 또는 일일 무료 사용량 제한에 도달했다는 뜻입니다. 자동으로 유료 결제가 발생했다는 뜻은 아닙니다. 잠시 기다린 뒤 다시 시도하거나 Google AI Studio에서 프로젝트의 사용량과 결제 설정을 확인하세요.

YouTube 공략 찾기에서는 3.5 화면 분석이 429로 실패하면 2.5 화면 분석을 시도하고, 그것도 실패하면 게임 이름 기반 직접 검색으로 계속 진행합니다.

### 오류 404

선택한 모델이 프로젝트에서 제공되지 않거나 종료된 경우 발생할 수 있습니다. 특히 2.5 Flash 종료 이후에는 검색 모델 설정을 후속 모델로 변경해야 합니다.
...
- AI 추론은 Google 서버에서 실행되므로 로컬 GPU에서 모델을 구동하지 않습니다.
- 화면은 버튼을 누를 때만 캡처합니다.
- 음성은 마이크 버튼을 누른 동안만 녹음합니다.
...
- 음성 입력을 실행하면 녹음한 음성이 Gemini API로 전송됩니다.
